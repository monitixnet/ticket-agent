...
Migration `0017_inventory_drop_watch.sql` seeds `Phantom of the Opera` at Segerstrom as a critical five-minute watch. Drop alerts currently fire for every confirmed ticket drop regardless of price; each Telegram includes observed price ranges and seat details. Drop watch has its own every-five-minute schedule (`*/5`) and does not run an all-events batch. Newly discovered sold-out events default to `medium` priority (30 minutes); `high` runs every 10 minutes, `low` runs weekly, and critical runs every five minutes. Explicit rules are always scoped by venue ID and exact saved show name; automatic sold-out monitoring needs no explicit rule. Segerstrom's `dropWatchBatchSize` is 20, enough to cover all current Phantom performances in one bounded pass. General inventory runs at `:07, :17, :27, :37, :47, :57`; listing watch runs at `:12, :32, :52`; discovery runs hourly at `:03` (`3 * * * *`). The Worker uses each venue's D1-configured local monitoring window as a hard curfew and records a structured no-work audit log when it is closed.

Migration `0037_add_availability_priority_policy.sql` enables Segerstrom Hall’s capacity-aware availability policy. It derives the hall capacity from the validated seat-map records, then calculates each performance’s available percentage from the authoritative SeatMe section-availability count. At **10% or less available** (90%+ unavailable), the performance enters the critical five-minute availability-watch lane. At **80% or more available**, it enters the low weekly lane. Halls without a validated mapped capacity do not receive automatic percentage-based priority. These are polling priorities only: Telegram drop alerts still require a confirmed `sold_out → available` transition.

A critical availability watch still sends a candidate Telegram whenever a newly observed block passes the configured quantity, contiguous-seat, same-price, and buffer rules. Its candidate message includes a clear label such as `Priority: CRITICAL — 92% SOLD OUT (8% available)`. This is distinct from the `TICKET DROP DETECTED` message, which is reserved for a confirmed sold-out-to-available change.
